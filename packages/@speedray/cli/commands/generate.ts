...
import {
  getCollection,
  getEngineHost
} from '@angular/cli/utilities/schematics';
import { DynamicPathOptions, dynamicPathParser } from '@angular/cli/utilities/dynamic-path-parser';
...
export default Command.extend({
  ...
  beforeRun: function(rawArgs: string[]) {
    ...
    if (/^\d/.test(rawArgs[1])) {
      SilentError.debugOrThrow('@speedray/cli/commands/generate',
        `The \`sr generate ${schematicName} ${rawArgs[1]}\` file name cannot begin with a digit.`);
    }
  ...
  run: function (commandOptions: any, rawArgs: string[]) {
    if (rawArgs[0] === 'module' && !rawArgs[1]) {
      throw 'The `sr generate module` command requires a name to be specified.';
    }
    ...
    const SchematicRunTask = require('@angular/cli/tasks/schematic-run').default;
