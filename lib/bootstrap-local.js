...
require.extensions['.ts'] = function (m, filename) {
  ...
  // We do touch `Speedray CLI` files anywhere though.
  if (!filename.match(/@speedray\/cli/) && filename.match(/node_modules/)) {
    if (oldRequireTs) {
...
if (!__dirname.match(new RegExp(`\\${path.sep}node_modules\\${path.sep}`))) {
  ...
  Module._load = function (request, parent) {
    ...
    } else if (request.startsWith('@speedray/cli/')) {
      // We allow deep imports (for now).
      // TODO: move tests to inside @speedray/cli package so they don't have to deep import.
